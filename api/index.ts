...
async function initDb() {
  ...
  if (count.length === 0 || count[0].values[0][0] === 0) {
    ...
    db.run('INSERT INTO strategies (id, name, description, config, is_public, user_id) VALUES (?, ?, ?, ?, ?, ?)',
      ['str_001', 'Moving Average Crossover', 'Buy when fast MA crosses above slow MA', JSON.stringify({ indicators: [{ type: 'SMA', period: 20 }, { type: 'SMA', period: 50 }] }), 1, 'usr_admin_001']);
    db.run('INSERT INTO strategies (id, name, description, config, is_public, user_id) VALUES (?, ?, ?, ?, ?, ?)',
      ['str_002', 'RSI Reversal', 'Buy when RSI oversold (<30)', JSON.stringify({ indicators: [{ type: 'RSI', period: 14 }] }), 1, 'usr_admin_001']);
    ...
    for (const sym of symbols) {
      ...
      db.run('INSERT INTO backtest_sessions (id, name, symbol, timeframe, start_balance, status, user_id, strategy_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
        [sid, `${sym.s} Demo`, sym.s, '1H', 10000, 'completed', 'usr_trader_001', 'str_001']);
      ...
      for (let i = 0; i < 500; i++) {
        ...
        db.run('INSERT OR IGNORE INTO candles (id, session_id, time, open, high, low, close, volume) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [`cand_${sid}_${i}`, sid, time, open, high, low, close, Math.floor(Math.random() * 10000) + 500]);
        price = close;
      }
    }
  }
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (err instanceof z.ZodError) return res.status(400).json({ success: false, error: 'Validation error', details: err.errors });
...
app.post('/api/sessions', authenticate, (req, res) => {
  try {
    ...
    const id = generateId();
    run('INSERT INTO backtest_sessions (id, name, symbol, timeframe, start_balance, user_id, strategy_id) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, data.name, data.symbol, data.timeframe, data.startBalance, req.user.id, data.strategyId || null]);
...
app.post('/api/sessions/:id/load-data', authenticate, (req, res) => {
  ...
  const symbols: Record<string, [number, number]> = { EURUSD: [1.0850, 0.0020], BTCUSD: [42000, 800], AAPL: [178, 3], GOLD: [2020, 20], GBPUSD: [1.2650, 0.0025], USDJPY: [150.50, 0.5], ETHUSD: [2250, 80], GOOGL: [140, 2.5], TSLA: [245, 8], SILVER: [24.5, 0.5] };
  ...
  for (let i = 0; i < cnt; i++) {
    ...
    run('INSERT OR IGNORE INTO candles (id, session_id, time, open, high, low, close, volume) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [`cand_${req.params.id}_${i}`, req.params.id, time, open, high, low, close, Math.floor(Math.random() * 10000) + 500]);
    price = close;
  }
...
app.post('/api/sessions/:id/trades', authenticate, (req, res) => {
  try {
    ...
    const id = generateId();
    run('INSERT INTO trades (id, session_id, type, entry_price, exit_price, entry_time, exit_time, quantity, stop_loss, take_profit, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, req.params.id, data.type, data.entryPrice, data.exitPrice || null, data.entryTime, data.exitTime || null, data.quantity, data.stopLoss || null, data.takeProfit || null, data.notes || null]);
...
app.post('/api/sessions/:id/run', authenticate, (req, res) => {
  ...
  if (candles.length < 50) return res.status(400).json({ success: false, error: 'Need at least 50 candles' });
  ...
  for (let i = slowPeriod; i < candles.length; i++) {
    const fastMA = candles.slice(i - fastPeriod, i).reduce((s: number, c: any) => s + c.close, 0) / fastPeriod;
    const slowMA = candles.slice(i - slowPeriod, i).reduce((s: number, c: any) => s + c.close, 0) / slowPeriod;
    const prevFastMA = candles.slice(i - fastPeriod - 1, i - 1).reduce((s: number, c: any) => s + c.close, 0) / fastPeriod;
    const prevSlowMA = candles.slice(i - slowPeriod - 1, i - 1).reduce((s: number, c: any) => s + c.close, 0) / slowPeriod;
  ...
  for (const t of trades) {
    run('INSERT INTO trades (id, session_id, type, entry_price, exit_price, entry_time, exit_time, quantity, pnl, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [generateId(), req.params.id, t.type, t.entryPrice, t.exitPrice, new Date().toISOString(), new Date().toISOString(), t.quantity, t.pnl, 'closed']);
  }
  ...
  run('UPDATE backtest_sessions SET status = ?, end_balance = ?, total_pnl = ?, total_pnl_pct = ?, total_trades = ?, winning_trades = ?, losing_trades = ?, win_rate = ? WHERE id = ?',
    ['completed', balance, totalPnl, (totalPnl / session.start_balance) * 100, trades.length, winningTrades, losingTrades, trades.length > 0 ? (winningTrades / trades.length) * 100 : 0, req.params.id]);
